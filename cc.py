...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Channel : @Extra_Earnn" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
def genmcard():
    ...
    rs = requests.post(sitex , headers=headers , cookies=cookies,data=payload)
    data = json.loads(rs.text)
    open("generated_cardby@ALLTYPECC.txt","w").write("")
    for i in range(1,10):
...
if opr == "1":
    print (rd + "[&] You selected first option ! \n\n")
    time.sleep(1)
    print (genscard())
elif opr == "2":
    ...
    print (bl + "[&] You selected third option !! \n\n")
    number = input(rd + "[$] Please Enter your card number :=> ")
    if detect_os() == "windows":
        popen = os.popen("node daxx\\val.js " + number).read()
        print (rd + popen + cv)
    else:
        popen = os.popen("node daxx/val.js " + number).read()
        print (bl + popen + cv)
